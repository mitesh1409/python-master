...
3.1 Python Lists  
3.2 Subsetting Lists  
3.3 List Manipulation  
3.4 How lists are stored?
3.5 Lists - available functions, commonly used functions  
3.6 Not suited for large datasets, for that we use Numpy/Pandas  
3.7 Summary
...
How?
List Creation:  
You can create a list using square brackets [] and store multiple data points/values within it.
...
indexing - accessing an element by providing an index  

slicing - accessing a range of elements by providing start:end indices  
...
So take care of this edge case while accessing lists.  
...
When you want to create a new list but with the same elements as in another list then  
you can use `list()` function or use slicing to select all the list elements explicitly.  
...
Are all the list elements stored in a contiguous memory locations?
